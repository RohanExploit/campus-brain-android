 ...
 */
class FtsSearch(private val db: BrainDb) {

    /** True once probed; false means the caller must fall back to [LikeSearch]. */
    val available: Boolean by lazy {
        runCatching {
            db.conn.query("SELECT rowid FROM chunks_fts WHERE chunks_fts MATCH 'a' LIMIT 1") { }
            true
        }.getOrElse { false }
    }
    ...
    fun search(queryText: String, topK: Int): List<RetrievedChunk> {
        ...
        return runCatching {
            db.conn.query(sql, { it.bindText(1, expr); it.bindLong(2, topK.toLong()) }) {
                RetrievedChunk(
                    id = it.getLong(0),
                    docId = it.getText(1),
 ...
 */
class LikeSearch(private val db: BrainDb) {
    fun search(queryText: String, topK: Int): List<RetrievedChunk> {
