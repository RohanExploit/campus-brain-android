...
object PremiseCheck {
    ...
    /**
     * A grade-shaped token: one or two letters carrying a +/- suffix, or a
     * bare pair of letters, which is the shape every real grade on this scale
     * has. A bare single letter is not accepted on purpose -- in "a grade" the
     * English article is a single letter, and refusing "A grade" as a false
     * premise would be a worse failure than answering it.
     */
    private const val TOKEN = """([A-Za-z]{1,2}[+‑-]|[A-Za-z]{2})"""
    ...
    /**
     * Two-letter English words that sit next to "grade" in ordinary questions
     * -- "no grade", "my grades", "grade in sem 3". Without this list the
     * check would announce that there is no "NO" grade, which is true and
     * useless. A token carrying +/- never needs the exemption: no English word
     * ends in one.
     */
